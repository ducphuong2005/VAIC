...
export function settingsPage() {
  ...
  const account = `<div id="settings-panel"><section class="settings-section"><h3>Thông tin tài khoản backend</h3><div class="avatar-setting"><span class="big-avatar">${initials}</span><div><p>Dữ liệu tài khoản đang hiển thị từ response đăng nhập/đăng ký backend.</p></div></div><div class="form-grid"><label>Họ và tên<input value="${user.fullName || ''}" readonly></label><label>Email<input type="email" value="${user.email || ''}" readonly></label><label>Số điện thoại<input value="${user.phone || ''}" readonly></label></div></section><section class="settings-section"><h3>Tùy chọn trải nghiệm</h3>${toggle('Gợi ý cá nhân hóa', 'Trạng thái này cần endpoint backend trước khi chỉnh sửa được.')}${toggle('Hiển thị trạng thái học tập', 'Chưa có endpoint backend để cập nhật tùy chọn này.', false)}</section><section class="settings-section danger-zone"><h3>Phiên đăng nhập</h3><p>Đăng xuất khỏi tài khoản hiện tại trên thiết bị này.</p><button class="btn btn-danger" type="button" data-logout-settings>${icon('lock', 16)} Đăng xuất</button></section></div>`;

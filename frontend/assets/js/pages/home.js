...
export async function homePage() {
  ...
  const skillsRaw = await optional(getSkillsInDemand(8), []);
  ...
  return `<div class="dashboard-grid"><div class="dash-left">${card('Mini-game nổi bật', game, { icon: 'game', action: '<a href="minigame.html" class="text-link">Xem tất cả ›</a>', className: 'game-card' })}</div><div class="dash-middle">${card('Nhóm ngành đang trending', trends, { icon: 'chart', className: 'trend-card trend-static-card' })}${card('Kỹ năng thị trường nổi bật', market, { icon: 'info', className: 'market-card' })}</div><div class="dash-right">${card('Career Compass AI', chat, { icon: 'bot', className: 'chat-card' })}</div></div>${card('Kỹ năng thị trường nổi bật từ data/jobs.csv', skillStrip(skillsRaw || []), { className: 'strength-card' })}`;
}
